...
def get_kpis(target_date: str = Query(...), db: Session = Depends(get_db)):
    ...
    # FIX: Alternate shift = attendance rows where reconciliation already flagged
    # issue="Alternate Shift" (assigned shift from Tata vs. actual worked shift
    # computed from real punch time). Previously this recomputed its own version
    # by comparing against the master employee shift, which drifted and gave
    # inconsistent results - now it's just a straight count of the same flag
    # reconciliation sets, so the KPI card and the detail table always agree.
    alternate_shift = sum(1 for a in attendance_records if a.issue == "Alternate Shift")
...
def upload_tata(file: UploadFile = File(...), force: bool = Form(False), db: Session = Depends(get_db)):
    ...
    for item in parsed:
        ...
        if emp:
            ...
            # FIX: previously this overwrote emp.shift (the master/assigned shift) on
            # EVERY row of every Tata upload, so it kept drifting to whatever day
            # happened to be processed last instead of staying stable. Master has no
            # shift data of its own - the per-day assigned shift already lives on the
            # TataAttendance row itself (see `shift` below), so there's nothing to
            # persist back to the employee record here.
...
def _reconcile_single_date(d: date, db: Session) -> Dict[str, Any]:
    ...
    for emp in employees:
        ...
        # Check alternate shift: compare the ASSIGNED shift (Tata's Shift column,
        # falling back to ESSL/master) against the shift actually WORKED, which is
        # derived from the real Tata IN punch time (falls back to ESSL IN time).
        # FIX: previously this compared the assigned shift against emp.shift (master),
        # which is a stale/unreliable field - it never actually checked the real
        # punch time against the assigned shift window at all.
        worked_shift = bucket_shift_from_time(tata_in or essl_in)
        if shift and worked_shift:
            assigned_shift_norm = normalize_shift(shift)
            worked_shift_norm = normalize_shift(worked_shift)
            if assigned_shift_norm in VALID_SHIFT_CODES and worked_shift_norm in VALID_SHIFT_CODES and assigned_shift_norm != worked_shift_norm and display_status in ["Present", "Single Punch"]:
    ...
    while chunk_start < end:
        ...
        while d < chunk_end:
            ...
            for emp in employees:
                ...
                # Check alternate shift: compare the ASSIGNED shift (Tata's Shift column,
                # falling back to ESSL/master) against the shift actually WORKED, which
                # is derived from the real Tata IN punch time (falls back to ESSL IN time).
                worked_shift = bucket_shift_from_time(tata_in or essl_in)
                if shift and worked_shift:
                    assigned_shift_norm = normalize_shift(shift)
                    worked_shift_norm = normalize_shift(worked_shift)
